...
export function useTimerContract() {
  ...
  const bettingContract = useAsyncInitialize(async () => {
    ...
    const contract = TimerContract.fromAddress(
      Address.parse("EQDXo18s0hQDTFoSadT_dAU7ubQAWKn8IPsNROg7IbjQWeR9")
    );
  ...
  return {
    ...
    sendAddress: async () => {
      const message: SecondAddressToPay = {
        secondAddressToPay: Address.parse("UQARwKlPwTlWMh-pBgqe-ChZa2BYG7QeeSJsPSeHvpu-B30j" as string),
